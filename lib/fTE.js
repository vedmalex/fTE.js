...
TemplateFactory.prototype.run = function(context, name) {
	var templ = this.ensure(name);
	var bc = this.blockContent(templ);
	return bc.run(context, bc.content, bc.partial);
};

TemplateFactory.prototype.renderFile = function(fileName, context) {
	var templ = this.ensure(fileName, true);
	var bc = this.blockContent(templ);
	return bc.run(context, bc.content, bc.partial);
};
...
TemplateFactory.prototype.clearCache = function(fn, list){
	...
	for(var i = 0, len = list.length; i < len; i++){
		delete this.cache[list[i].name];
		delete this.cache[list[i].absPath];
	}
}
	...
	for (var i = 0, len = this.root.length; i < len; i++) {
		...
		if (fw) {
			if (!this.watchTree[fw]) {
				this.watchTree[fw] = {
					watcher: fs.watch(fw, { persistent: false }, ()=> {
						debugger;
						var list = this.watchTree[fw].list;
